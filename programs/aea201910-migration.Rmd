...
table.aea.file.ext %>% 
	...
	mutate(sw=if_else(Pct>1,Software,"Other")) %>% 
...
aea.complete.programs %>% group_by(Software,doi) %>% 
	summarize(count=n()) %>% group_by(Software) %>% 
	summarize(count=n()) %>% arrange(desc(count)) %>% ungroup() -> table.aea.pkgs.ext
table.aea.pkgs.ext %>% summarize(pkgs = sum(count)) -> sum.pkgs.programs
table.aea.pkgs.ext$Pct = round(table.aea.pkgs.ext$count / as.numeric(sum.pkgs.programs) * 100,2)
table.aea.pkgs.ext %>% 
	mutate(Software=if_else(Software=="General","Other",Software)) %>%
	mutate(Software=if_else(Software=="Unknown","Other",Software)) %>%
	mutate(sw=if_else(Pct>1,Software,"Other")) %>% 
	group_by(sw) %>% summarise(count=sum(count),Pct=sum(Pct)) %>% 
	arrange(desc(Pct)) %>% 
	ggplot(aes(sw,count)) +
